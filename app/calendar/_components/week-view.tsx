        ...
        {weekDays.map((day, index) => {
          ...
          return (
            ...
            >
              ...
              <Link href={`/calendar/event?date=${day.toISOString()}`} onClick={(e) => e.stopPropagation()}>
        ...
        {hours.map((hour) => (
          <div key={hour} className="grid grid-cols-8 border-b">
            ...
            {weekDays.map((day, dayIndex) => {
              ...
              return (
                <div key={dayIndex} className="relative min-h-[60px] p-1 border-r" onClick={() => onDateClick(day)}>
                  {dayEvents.map((event) => (
                    <div
                      key={event.id}
                      className={`absolute left-1 right-1 rounded-sm px-2 py-1 text-xs ${
                        event.type === "meeting"
