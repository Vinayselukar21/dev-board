...
import { CalendarIcon, Clock, Folder, Locate, MapPin, Trash2, User } from "lucide-react"
...
export function EventDetailsDialog({ event, onEdit, trigger }: EventDetailsDialogProps) {
  ...
  useEffect(() => {
    const selectedEvent : CalendarEvent | undefined = eventsData.find((e: CalendarEvent) => e.id === event?.id);
      if (selectedEvent) {
        ...
        setParticipants(selectedEvent.participants?.map((p: any) => p.workspaceMemberId) || [])
      }
  }, [event])
      ...
      <DialogContent className="sm:max-w-[550px]">
            <DialogHeader>
              <div className="flex items-center justify-between">
                ...
                <Badge className={getEventTypeColor(eventType) + " mr-5"}>{eventType}</Badge>
              </div>
            ...
            <div className="space-y-4">
              ...
              {projectId && (
                <div className="flex items-center gap-2">
                  ...
                  <span>{activeWorkspace?.projects?.find((p: Project) => p.id === projectId)?.name}</span>
                </div>
              )}

              <div className="flex items-center gap-2 flex-wrap">
              {participants.length > 0 && (
                activeWorkspace?.members?.map((m: any) =><div className="flex items-center gap-2 w-fit bg-muted/80 p-2 rounded-md text-sm" key={m.id}>
